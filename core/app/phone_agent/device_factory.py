...
class DeviceFactory:
    ...
    def module(self):
        ...
        if self._module is None:
            if self.device_type == DeviceType.ADB:
                ...
                self._module = hdc
            elif self.device_type == DeviceType.VIRTUAL_DISPLAY:
                from phone_agent import virtual_display

                self._module = virtual_display
            else:
    ...
    def get_connection_class(self):
        ...
        if self.device_type in (DeviceType.ADB, DeviceType.VIRTUAL_DISPLAY):
...
def set_virtual_display(
    ...
    """Select and configure the scrcpy virtual-display backend."""
    global _device_factory
    from phone_agent.virtual_display import configure

    configure(display_id, window_title, screen_size, window_pid, device_id,
              frame_path, frame_producer_pid)
    _device_factory = DeviceFactory(DeviceType.VIRTUAL_DISPLAY)
