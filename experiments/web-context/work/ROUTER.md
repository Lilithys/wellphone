...
- 后续真实 plan 的一条完整任务及三条边界样例已通过（用户在对话提供输出；边界为信息缺失、未支持的提醒、否定指令）。不将这些有限样例扩大为一般语义正确性保证。
- 19:53–19:55真实 LLM 双通道已通过：`outputs/router-20260908-195319-j09rmkyh/result.json` 中 `source=llm`、`state=COMPLETED`，事件30新建并回读，13次主屏采样正常；关联 `outputs/router-gui-20260908-195359-ph2szmg6/result.json` 中 GUI 4步完成、91次主屏采样通过、人工观察正常、副屏48清理。已冻结为 `baseline-hybrid-agent-v1`，具体证据哈希与源码快照见[验收基线](../baseline/hybrid-agent-v1.md)。上面的失败记录是历史过程，不是当前最终验收状态。
