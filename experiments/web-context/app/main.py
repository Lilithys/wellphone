...
Examples:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Use an existing scrcpy virtual display (must be greater than 0)",
    )

    parser.add_argument(
        "--virtual-display-window-title",
        type=str,
        default="Wellphone-Agent",
        help="Exact title of the scrcpy window used for virtual-display frames",
    )

    parser.add_argument(
        "--virtual-display-window-pid",
        type=int,
        default=None,
        help="scrcpy process id used to locate its macOS window reliably",
    )
...
def main():
    ...
    if args.virtual_display_id == 0:
        print("Error: display 0 is the user's main screen and cannot be used here.")
        sys.exit(2)
    ...
    if args.virtual_display_id is not None:
        if device_type != DeviceType.ADB:
            print("Error: virtual display mode currently requires --device-type adb")
            sys.exit(2)
        from phone_agent.device_factory import set_virtual_display

        set_virtual_display(
            display_id=args.virtual_display_id,
            window_title=args.virtual_display_window_title,
            window_pid=args.virtual_display_window_pid,
