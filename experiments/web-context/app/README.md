...
### Android 虚拟显示（实验性，macOS）

同机连续打字测试请优先使用已接入焦点隔离实验版的新入口：

```bash
python3 /Users/yishanma/Documents/Codex/2026-09-03/https-app-notion-com-p-take/work/run_autoglm_focus.py --preflight
python3 /Users/yishanma/Documents/Codex/2026-09-03/https-app-notion-com-p-take/work/run_autoglm_focus.py
```

首条检查副屏设置 → 关于手机 → 返回的实时截图、不调用模型；第二条正式联调，API key 可在 Terminal 隐藏输入。
新入口已改为原生副屏视频取帧，需本机 ffmpeg，不依赖 Mac 屏幕录制权限；不会截图手机主屏。
下方是底层手动附着示例，**原版 scrcpy 并不具备本项目已验证的禁止抢顶层焦点配置，不要把该示例当作无干扰验收入口**。

先创建一个固定尺寸的 scrcpy 虚拟显示，并记下日志中的 display id：

```bash
scrcpy --new-display=1080x2400/420 \
  --display-ime-policy=local \
  --no-audio \
  --window-title=Wellphone-Agent \
...
python main.py \
  ...
  --virtual-display-id DISPLAY_ID \
  --virtual-display-window-title Wellphone-Agent \
  --max-steps 10 \
  "打开设置并找到关于手机页面"
```

虚拟显示模式会拒绝 display 0，并将点击、滑动和按键定向到指定显示。
上方旧式手动附着仍从 scrcpy 窗口取帧，可能因本机窗口缩略图而失败；新入口绕开了此路径。为避免切换全局输入法，
文字输入和清空文本暂时禁用，等待单独的并发输入验收。
