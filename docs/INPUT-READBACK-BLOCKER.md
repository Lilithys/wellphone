...
结论：已定位本机客户端层级读取超时，交付入口已改用同权限、按副屏限定的另一条读取路径。新解析器在同一真机输入框连续三次通过；随后 2026-09-10 10:51 的监督式流程完成输入和单次发送，主屏观察正常。仍不宣称对方收件回执或长期稳定。
...
- 诊断副屏已移除，抖音两项音频设置已恢复并回读。后续完整验收记录显示模型调用、输入和单次发送均完成；主屏观察为正常、无额外声音，副屏和音频已清理。收件回执与连续火花仍未验证。

正常运行命令不变，见 [当前入口](BASELINE-ENTRY.md)。完整监督式流程已有一轮通过；若存在旧输入/发送尝试，必须先核对真实状态，不删除日志或换目录绕过防重复机制。
